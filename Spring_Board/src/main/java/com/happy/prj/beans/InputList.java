...
public class InputList {
	...
	// 날짜 포멧 변경(2019-02-02 2:11:15.0)->2019-02-02
...
//		<img alt="리플" src="./image/reply.png">
	...
	private String listFormat(Answerboard_DTO dto) {
		...
		if(mem.getId().trim().equalsIgnoreCase(dto.getId().trim())||mem.getAuth().trim().equalsIgnoreCase("A")) {
			sb.append("<input class='btn btn-primary' type='button' value='글삭제하기' onclick='del(\""+dto.getSeq()+"\")'/>");
		}
		if(mem.getAuth().trim().equalsIgnoreCase("U")) {
			sb.append("<input class='btn btn-primary' type='button' value='글답글' onclick='reply(\""+dto.getSeq()+"\")'/>");
		}
