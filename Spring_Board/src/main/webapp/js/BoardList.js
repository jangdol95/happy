...
// alert(obj.title);
        ...
        "<div class='form-group'>"+
           "<label>내용</label>"+
           "<textarea rows='7' class='form-control' name='content' id='mocontent' required='required'>"+obj.content+"</textarea>"+
        "</div>"+   
...
var ajaxReply=function(seq){
	$.ajax({
		...
		success:function(obj){
			...
			"</div>"+
			"<div class='form-group'>"+
			"<label>답글할 글 조회수</label><br>"+
			"<p class='form-control'>"+obj.readcount+"</p>"+
			"</div>"+
			"<div class='form-group'>"+
			"<label>답글할 글 작성일</label><br>"+
			"<p class='form-control'>"+obj.regdate+"</p>"+
			"</div>"+
			"<div class='form-group'>"+                                                                                              
			"<label>답글할 글 작성자</label><br>"+                                                                                   
			"<p class='form-control'>"+obj.memid+"</p>"+                                                                                      
			"</div>"+                                                                                                                   
			"<div class='form-group'>"+                                                                                                 
			"<label id='titl'>제목(원본글)</label><br>"+                                                                             
			"<input type='text' class='form-control' id='title' value='"+obj.title+"' name='title' required='required' onclick='chk()'>"+   
			"</div>"+		                                                                                                              
			"<div class='form-group'>"+                                                                                                 
			"<label id='con'>내용(원본)</label>"+                                                                                    
			"<input type='hidden' id='hiddenContent' value='"+obj.content+"'>"+                                                                 
			"<textarea rows='7' id='txtArea' class='form-control' name='content' onclick='chk()'>"+obj.content+"</textarea>"+                   
			"</div>"+                                                                                                                   
			"<div class='modal-footer'>"+                                                                                               
			"<input type='button' value='답글작성' class='btn btn-warning' onclick='replied()'>"+                                    
			"<input type='reset' value='초기화' class='btn btn-warning' onclick='titleReset()'>"+                                                           
			"<button type='button' class='btn btn-default' data-dismiss='modal'>닫기</button>"+                                      
			"</div>";
...
//	alert(title+":"+txtArea);
	if(title==""||txtArea==""){
		swal("답글 작성오류","제목 혹은 내용을 작성해주세요.");
	}else{
		var titl=document.getElementById("titl").innerHTML;
		if(titl=="제목"){
			frm.submit();
		}else{
			swal("답글 작성오류","답글을 먼저 작성해주세요.");
		}
	}
}
function chk(){
